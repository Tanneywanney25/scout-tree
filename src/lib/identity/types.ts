...
export interface ResolutionResult {
  ...
  elapsedMs: number;
  /** Wall-clock per major phase (anchor providers, graph fetch, traversal,
   *  fallbacks…), ms — the profile that shows WHERE a slow search spent its
   *  time, so future bottlenecks are diagnosable from any run's result. */
