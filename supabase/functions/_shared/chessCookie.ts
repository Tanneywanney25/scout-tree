...
export function parseCookiePairs(cookieHeader: string | null | undefined): Map<string, string> {
...
export function mergeCookies(base: Map<string, string>, updates: Map<string, string>): Map<string, string> {
...
export function formatCookieHeader(map: Map<string, string>): string {
...
export function sessionCookieSummary(map: Map<string, string>): string {
...
export async function getCachedChesscomCookie(): Promise<CachedCookie | null> {
  ...
  try {
    ...
    const rows = (await res.json()) as Array<{ cookie?: string; source?: string; updated_at?: string }>;
...
export async function putChesscomCookie(cookie: string, source: string): Promise<boolean> {
  const rest = supabaseRest();
...
// ---------------------------------------------------------------------------
// Reader — cache first, env fallback. Memoized so a whole crawl (one call per
// schoolmate) costs at most one DB round-trip per REFRESH_MEMO_MS window.
// ---------------------------------------------------------------------------

export interface ResolvedCookie {
  cookie: string;
  source: "cache" | "env" | "none";
  /** Age of the cached value in ms, when it came from the cache. */
  ageMs?: number;
}

const REFRESH_MEMO_MS = 60_000;
...
let cacheMemo: { at: number; value: CachedCookie | null } | null = null;

async function cachedCookieMemoized(): Promise<CachedCookie | null> {
  const now = Date.now();
  if (cacheMemo && now - cacheMemo.at < REFRESH_MEMO_MS) return cacheMemo.value;
...
export async function readChesscomSessionCookie(): Promise<ResolvedCookie> {
...
export function resetCookieMemo(): void {
  cacheMemo = null;
}
...
export interface RefreshResult {
  ok: boolean;
  /** How the stored cookie was obtained. */
  source: "keepalive" | "login" | "none";
...
async function credentialLogin(
  ...
  if (!turnstile) {
    log(
      "login: no CHESSCOM_TURNSTILE_TOKEN supplied. chess.com gates /login with Cloudflare Turnstile, " +
  ...
  if (res.status === 200 || /\/login/i.test(location)) {
    log(
      `login: /login_check did not establish a session (HTTP ${res.status}${location ? `, →${location}` : ""}). ` +
 ...
 */
export async function refreshChesscomCookie(log: Logger = () => {}): Promise<RefreshResult> {
  ...
  const msg =
    "Could not refresh the Chess.com session. Keep-alive found no valid session and credential login was rejected " +
