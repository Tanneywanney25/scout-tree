...
Original-Path:  supabase/functions/resolve-identity/uscf.ts
Change-Type:    MODIFIED FILE (full redesign version)
------------------------------------------------------------
WHAT:  The MUIR/US Chess helpers with the redesign's ADDITIVE functions:
       directoryMemberSearch (state-only browse supported), searchRatedEvents,
       and fetchEventRoster. Existing helpers untouched.
WHY:   Back the searchMembers / searchEvents / eventRoster edge modes.
DEPENDS-ON:     the existing throttled MUIR client in the same module.
DEPENDED-ON-BY: supabase/functions/resolve-identity/index.ts (redesign modes).
RESTORE:        Additive only — see uscf.ts.delta.patch for the exact added
                helpers.
------------------------------------------------------------
