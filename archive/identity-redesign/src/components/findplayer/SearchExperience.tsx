...
Original-Path:  src/components/findplayer/SearchExperience.tsx
Change-Type:    MODIFIED FILE (full redesign version)
------------------------------------------------------------
WHAT:  The live "searching" screen. Redesign delta vs the live version is
       small and additive: it imports SearchPhaseStrip and renders it above
       the reasoning feed to project provider events onto a phase ladder
       (anchor -> trace -> web -> school -> name search).
WHY:   Give the long Phase-B wait a legible "where are we / what's left" map
       instead of only a scrolling log.
DEPENDS-ON:     src/components/findplayer/SearchPhaseStrip.tsx.
DEPENDED-ON-BY: src/pages/FindPlayer.tsx (both live and redesign variants render it).
RESTORE:        The ONLY redesign change is: (1) `import SearchPhaseStrip from
                "./SearchPhaseStrip";` and (2) rendering
                `{providerStatusProp && <SearchPhaseStrip providerStatus={providerStatusProp} />}`
                just above the live reasoning feed. See
                SearchExperience.tsx.delta.patch for the exact hunk. Restore
                SearchPhaseStrip.tsx to src/ first.
------------------------------------------------------------
...
import SearchPhaseStrip from "./SearchPhaseStrip";
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-background/95 backdrop-blur-xl">
      ...
      <div className="w-full max-w-xl px-6 text-center">
        ...
        {/* --- Phase map: where we are in the ladder, what's left --- */}
        {providerStatusProp && <SearchPhaseStrip providerStatus={providerStatusProp} />}
