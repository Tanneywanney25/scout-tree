...
Original-Path:  src/components/findplayer/usStates.ts
Change-Type:    NEW FILE
------------------------------------------------------------
WHAT:  Static list of US states (code + label) used to populate the state
       dropdowns in the name/browse/tournament doors.
WHY:   Shared constant for state filtering across the entry doors.
DEPENDS-ON:     nothing.
DEPENDED-ON-BY: src/components/findplayer/DiscoveryTabs.tsx,
                src/components/findplayer/TournamentPicker.tsx.
RESTORE:        Copy the source below to src/components/findplayer/usStates.ts.
------------------------------------------------------------
...
/** US states + DC for the directory filters (USCF StateRep codes). */
